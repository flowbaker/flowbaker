...
func NewExecutorTaskPublisher(deps ExecutorTaskPublisherDependencies) ExecutorTaskPublisher {
	return &executorTaskPublisher{
		client: deps.Client,
	}
}

func (p *executorTaskPublisher) EnqueueTask(ctx context.Context, workspaceID string, task Task) error {
	if workspaceID == "" {
...
func (p *executorTaskPublisher) EnqueueTaskAndWait(ctx context.Context, workspaceID string, task Task) ([]byte, error) {
	if task == nil {
