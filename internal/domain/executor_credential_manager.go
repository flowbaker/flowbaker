...
type executorCredentialManager struct {
	...
	decryptionSvc ExecutorCredentialDecryptionService
}

func NewExecutorCredentialManager(client flowbaker.ClientInterface, decryptionSvc ExecutorCredentialDecryptionService) ExecutorCredentialManager {
...
func (e *executorCredentialManager) GetOAuthAccount(ctx context.Context, oauthAccountID string) (OAuthAccount, error) {
	workflowExecutionContext, ok := GetWorkflowExecutionContext(ctx)
	if !ok {
		return OAuthAccount{}, fmt.Errorf("workflow execution context is required")
	}

	response, err := e.client.GetOAuthAccount(ctx, workflowExecutionContext.WorkspaceID, oauthAccountID)
	if err != nil {
		return OAuthAccount{}, fmt.Errorf("failed to get OAuth account: %w", err)
	}

	// Convert flowbaker.OAuthAccount to domain.OAuthAccount
	domainAccount := OAuthAccount{
		ID:        response.OAuthAccount.ID,
		UserID:    response.OAuthAccount.UserID,
		OAuthName: response.OAuthAccount.OAuthName,
		OAuthType: OAuthType(response.OAuthAccount.OAuthType),
...
func (e *executorCredentialManager) GetFullCredential(ctx context.Context, credentialID string) (Credential, error) {
	workflowExecutionContext, ok := GetWorkflowExecutionContext(ctx)
	if !ok {
		return Credential{}, fmt.Errorf("workflow execution context is required")
	}

	encryptedCred, err := e.client.GetFullCredential(ctx, workflowExecutionContext.WorkspaceID, credentialID)
	if err != nil {
		return Credential{}, fmt.Errorf("failed to get encrypted full credential: %w", err)
	}
	...
	decryptedBytes, err := e.decryptionSvc.DecryptCredential(credential)
	if err != nil {
		return Credential{}, fmt.Errorf("failed to decrypt full credential: %w", err)
	}

	// Unmarshal the decrypted bytes into full Credential struct
	var fullCredential Credential
	if err := json.Unmarshal(decryptedBytes, &fullCredential); err != nil {
		return Credential{}, fmt.Errorf("failed to unmarshal full credential: %w", err)
	}
...
type ExecutorCredentialGetter[T any] struct {
	manager ExecutorCredentialManager
}
