...
// IntegrationMemory Implementation
...
func (i *RedisIntegration) StoreConversation(ctx context.Context, request domain.StoreConversationRequest) error {
	...
	conversationJSON, err := json.Marshal(request.Conversation)
	if err != nil {
		return fmt.Errorf("failed to marshal conversation: %w", err)
	}

	// Store main conversation
	conversationKey := fmt.Sprintf("conversation:%s:%s", request.Conversation.SessionID, request.Conversation.ConversationID)
	var expiration time.Duration
	if request.TTL > 0 {
		expiration = time.Duration(request.TTL) * time.Second
	}

	err = i.client.Set(ctx, conversationKey, conversationJSON, expiration).Err()
	if err != nil {
		return fmt.Errorf("failed to store conversation: %w", err)
	}

	// Add to sorted set for time-based retrieval
	indexKey := fmt.Sprintf("conversations:index:%s", request.Conversation.SessionID)
	score := float64(request.Conversation.CreatedAt.Unix())
	err = i.client.ZAdd(ctx, indexKey, redis.Z{
		Score:  score,
		Member: conversationKey,
	}).Err()
	if err != nil {
		return fmt.Errorf("failed to update conversation index: %w", err)
	}

	// Store embedding if provided (for semantic search)
	if len(request.Embedding) > 0 {
		embeddingJSON, err := json.Marshal(request.Embedding)
		if err != nil {
			return fmt.Errorf("failed to marshal embedding: %w", err)
		}
		embeddingKey := fmt.Sprintf("embedding:%s", conversationKey)
		err = i.client.Set(ctx, embeddingKey, embeddingJSON, expiration).Err()
		if err != nil {
			log.Error().Err(err).Msg("Failed to store conversation embedding")
		}
	}
...
func (i *RedisIntegration) RetrieveConversations(ctx context.Context, filter domain.ConversationFilter) ([]domain.AgentConversation, error) {
	indexKey := fmt.Sprintf("conversations:index:%s", filter.SessionID)

	// Calculate score range based on time filter
	minScore := "-inf"
	maxScore := "+inf"
	...
	// Get conversation keys from sorted set
	limit := filter.Limit
	if limit <= 0 {
		limit = 10
	}

	keys, err := i.client.ZRevRangeByScore(ctx, indexKey, &redis.ZRangeBy{
		Min:   minScore,
		Max:   maxScore,
		Count: int64(limit),
	}).Result()
	if err != nil {
		return nil, fmt.Errorf("failed to retrieve conversation keys: %w", err)
	}

	conversations := make([]domain.AgentConversation, 0, len(keys))
	for _, key := range keys {
		conversationJSON, err := i.client.Get(ctx, key).Result()
		if err != nil {
			if err == redis.Nil {
				continue
			}
			return nil, fmt.Errorf("failed to retrieve conversation %s: %w", key, err)
		}

		var conversation domain.AgentConversation
		err = json.Unmarshal([]byte(conversationJSON), &conversation)
		if err != nil {
			log.Error().Err(err).Str("key", key).Msg("Failed to unmarshal conversation")
			continue
		}

		// Apply additional filters
		if filter.WorkspaceID != "" && conversation.WorkspaceID != filter.WorkspaceID {
			continue
		}
